...
class Variable(object):
    ...
    def calculate_marginal_probability(self):
        ...
        for i, v in enumerate(self.probability_table.items()):
            marginalTrue = v[1][0]

            if (self.parents != None):
                for j, v2 in enumerate(self.parents):
                    marginalTrue *= v2.get_marginal_probability(v[0][j])
            true += marginalTrue

        self.marginal_probabilities = (true, 1-true)
